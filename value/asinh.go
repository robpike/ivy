...
func acosh(c Context, v Value) Value {
	...
	if compare(v, 1) < 0 {
		return complexAcosh(c, NewComplex(c, v, zero))
	}
...
func atanh(c Context, v Value) Value {
	...
	if compare(v, -1) <= 0 || 0 <= compare(v, 1) {
		return complexAtanh(c, NewComplex(c, v, zero))
	}
...
func floatAcosh(c Context, x *big.Float) *big.Float {
	if x.Cmp(floatOne) < 0 {
		c.Errorf("real acosh out of range [1, +∞ )")
	}
	z := newFloat(c).Set(x)
	z.Mul(z, x)
	z.Sub(z, floatOne)
...
func floatAtanh(c Context, x *big.Float) *big.Float {
	if x.Cmp(floatMinusOne) <= 0 || 0 <= x.Cmp(floatOne) {
		c.Errorf("real atanh out of range (-1, 1)")
	}
...
func complexAsinh(c Context, x Complex) Complex {
	z := x.mul(c, x)
	z = z.add(c, NewComplex(c, one, zero))
	z = complexSqrt(c, z)
	z = z.add(c, x)
...
func complexAcosh(c Context, x Complex) Complex {
	z := x.mul(c, x)
	z = z.sub(c, NewComplex(c, one, zero))
	z = complexSqrt(c, z)
	z = z.add(c, x)
	return complexLog(c, z)
}
