...
func (v *Vector) Len() int { return len(v.slice) }

// At returns the i'th element of v.
func (v *Vector) At(i int) Value { return v.slice[i] }

// Set sets v[i] = x.
func (v *Vector) Set(i int, x Value) { v.slice[i] = x }

// All returns all the elements in v, for reading.
func (v *Vector) All() []Value { return v.slice[:len(v.slice):len(v.slice)] }

// WriteAll returns all the elements in v, for writing.
func (v *Vector) WriteAll() []Value { return v.slice }

// Slice returns a slice v[i:j], for reading.
func (v *Vector) Slice(i, j int) []Value { return v.slice[i:j:j] }
